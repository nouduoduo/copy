...
function progressivelyDecreasing() {
    ...
    $(".increasing").on("click",function () {
        ...
        //设置总价
        var other = $(this).parents("tr").siblings().children(".sinToPrice").text();
        var other_d = parseFloat(other.slice(other.indexOf("￥") + 1));
        var current = $(this).parent().siblings(".sinToPrice").text();
        var current_d = parseFloat(current.slice(current.indexOf("￥") +1));
        console.log(other_d , current_d );
        if($("tbody tr").length == 1){
            $(".monney").text("￥"+ current_d .toFixed(2));
        }else {
            $(".monney").text("￥"+ ( other_d + current_d ).toFixed(2));
        }
    });
